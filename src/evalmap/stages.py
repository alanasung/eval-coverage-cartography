"""Experiment stages for When Benchmarks Saturate, What Did They Never Measure."""
...
from .evalmap._util import ensure_dir, stage_result, write_json
...
def stage_fit(cfg: DictConfig, run_dir: Path) -> dict[str, Any]:
    ...
    write_json(out / "fit.json", {"status": "ok", "claim_ok": False, "note": "pilot fit stub"})
...
def stage_report(cfg: DictConfig, run_dir: Path) -> dict[str, Any]:
    ...
    write_json(out / "report.json", {"headline": "When Benchmarks Saturate, What Did They Never Measure", "claims_gated": True})
