# Design Document: When Benchmarks Saturate, What Did They Never Measure

## 1. Purpose

This document is the method contract for **When Benchmarks Saturate, What Did They Never Measure**. It specifies what is
...
One-liner: Use activation geometry to map which capabilities a eval suite actually stresses.

Hypothesis: Use activation geometry to map which capabilities a eval suite actually stresses.
...
|---|---|---|
| Config schema | `evalmap.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `evalmap.configs.loader` | Hydra compose API |
| Device | `evalmap.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `evalmap.models.registry` | Pinned revisions + hardware fit |
| Hooks | `evalmap.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `evalmap.models.generation` | Chat template path recording |
| Data | `evalmap.data` | Synthetic items, splits, manifests |
| Evaluation | `evalmap.evaluation` | Metrics, failure harness, protocols |
| Ablation | `evalmap.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `evalmap.reporting` | Aggregate → MD/TeX/figures |
| Cache | `evalmap.cache` | Versioned artifact store |
...
`src/evalmap/evalmap/` are preserved across regeneration.
